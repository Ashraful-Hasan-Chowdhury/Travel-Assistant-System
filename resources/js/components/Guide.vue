...
{{ userPosition }}</small></center>
            ...
            <div class="card-header py-3 bg-primary">
              <h5 class="m-0 font-weight-bold">Nearby Guides</h5>
            </div>
            <div class="card-body">
                ...
                <table class="table table-bordered"  width="100%" cellspacing="0">
                  ...
                  <tbody>
                    ...
                    <tr v-for="(place,i) in found.data" :key="place.id">
                      ...
                      <td>{{ place.name }}</td>
                      <td>{{ place.address }}</td>
                      <td><button class="btn btn-sm btn-info" @click.prevent="viewPhone(place.id)">
                        Phone Number
                      </button></td>
...
export default {
  ...
  methods: {
    ...
    getResults(page = 1) {
      axios.post('show-guide?page='+page,{
                    lat:this.center.lat,
                    lng:this.center.lng
                })
        .then(response => {
    ...
    viewDetails(placeid){
        axios.get('guide-details/'+placeid)
                .then(response => { 
    ...
    viewPhone(placeid){
      this.$toaster.info('Phone number is being sent to your email!', {timeout: 8000})
        axios.get('phone/'+placeid)
                .then(response => { 
                  this.$toaster.success('Phone number is sent to your mail successfully!', {timeout: 8000})
                });
    },
